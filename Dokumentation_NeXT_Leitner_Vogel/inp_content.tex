...
In der Tabelle \ref{Stakeholder-Analyse} unten ist das Ergebnis unserer Stakeholder-Analyse zu sehen.
\begin{table}[H]
...
\label{Stakeholder-Analyse}
	\end{table}
Die Einschätzung der Einstellung in Tabelle \ref{Stakeholder-Analyse} geht von sehr negativ über neutral bis zu sehr positiv. Der Einfluss wiederum wird mit einer Skala von 1-10 beschrieben, wobei 1 keinen bzw. sehr wenig Einfluss repräsentiert und 10 sehr starken. Auch die Nähe zum Projekt wird von 1-10 skaliert, 1 repräsentiert weit entfernt vom Projekt und 10 sehr nah.
...
\begin{figure}[H]
	...
	\captionbelow{Stakeholder-Diagramm -->	In diesem Diagramm ist das Ergebnis der Tabelle \ref{Stakeholder-Analyse} graphisch dargestellt. Die Größe der Kreise Zeigt den Einfluss, die Länge der Linien zwischen Stakeholder und NeXt die Nähe und die Zeichen (++ --> sehr positiv, ~ --> neutral, -- --> sehr negativ) unter dem Namen des Stakeholders dessen Nähe zum Projekt. }
\end{figure}
...
Eine Spiel-Engine ist eine Art Framework, welches spezielle für Computerspiele entwickelt wurde. Die Engine ist zuständig für den Spielverlauf aber auch für die visuelle Darstellung des Spielablaufs. Viele Spiel-Engines liefern auch eine Entwicklungsumgebung mit, diese liefert die Werkzeuge für die Entwicklung der jeweiligen Applikation mit sich. \cite{Spiel-Engine}

\subsection{Unity-Engine}
Auf anraten unseres Projektpartners verwendeten wir die Unity-Enigne, da auch das Unternehmen selbst damit arbeitet. Die Engine liefert auch eine Entwicklungsumgebung mit sich, welche sehr angenehm zum arbeiten ist. Zusätzlich ist es eine sehr weit verbreitete Spiel-Engine zu der es auch äußerst hilfreiches Lehrmaterial gibt, Unity selbst bietet auch eine Vielzahl an Lernvideos an.
...
Im folgenden Abschnitt sind einige Technische Eigenschaften von Unity erklärt, es werden jedoch nicht alle beschrieben da dies den Rahmen dieses Projekts sprängen würde.

\textbf{Grafik}\\
Unity ist auf dem neusten Stand der Technik und unterstützt die verschiedensten Techniken die derzeit in der Spielentwicklung benötigt werden. Außerdem kann man auch die eingebauten Beleuchtungseffekte selbst bearbeitet werden, dies erweitert die Möglichkeiten der Engine nochmals signifikant.
...
\begin{itemize}
	...
	\item Boo
\end{itemize}
Diese sind notwendig um Skripte zu erstellen, welche die von Unity eingebauten Mechanismen erweitern. Da wir im Unterricht bereits mit C\# gearbeitet haben wählten wir auch diese.
...
Als Entwicklungsumgebung wählten wir Unity-Editor da sie auch von Unity Technologies entwickelt wurde und somit speziell für die Unity-Engine gemacht wurde. Sie wurde basierend auf gängigen 3D-Animationsprogrammen designet. Sie ist sehr einfach zu bedienen und ermöglicht auch das importieren von Werkzeugen mittels eines einfachen Mausklicks. In dieser Entwicklungsumgebung können alle für die Spielentwicklung wichtigen Arbeiten verrichtet werden, nur für das Programmieren der Scripte verwendeten wir Visual Studio. \cite{Unity-Engine}

\subsection{Visual Studio}
Wie bereits oben kurz erwähnt verwendeten wir für die Programmierung von Scripten Visual Studio Community 2017, welche für Studenten kostenlos ist und wir auch schon im Unterricht zum entwickeln von Apllikatione mit C\#  verwendeten. 
